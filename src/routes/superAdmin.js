...
router.post("/centers", async (req, res, next) => {
  try {
    ...
    // Buat folder Drive untuk center ini
    let driveFolderId = null;
    try {
      driveFolderId = await createCenterFolder(name.trim());
    } catch (driveErr) {
      logger.error("Failed to create Drive folder for center", { name, error: driveErr.message });
      return res.status(502).json({ success: false, message: "Failed to create Drive folder. Please try again." });
    }
...
router.post("/admins", async (req, res, next) => {
  try {
    ...
    if (!email?.trim() || !name?.trim() || !center_id) {
    ...
    const result = await query(
      ...
      [email.trim().toLowerCase(), name.trim(), center_id],
    );
    ...
    logger.info("Admin pre-registered", { adminId: result.rows[0].id, email, createdBy: req.user.id });
...
router.get("/download/enrollments", async (req, res, next) => {
  try {
    const { whereClause, values } = buildWhere([
      { col: "e.center_id", val: req.query.center_id ? parseInt(req.query.center_id) : undefined },
