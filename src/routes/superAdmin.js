...
const { validate, createCenterBody, updateCenterBody, createAdminBody, monitoringUploadQuery, monitoringActivityQuery, downloadEnrollmentsQuery, idParam, paginationQuery } = require("../validators");
...
router.get("/admins", validate(paginationQuery, "query"), async (req, res, next) => {
  try {
    const { page, limit, offset } = parsePagination(req.query);
    const { whereClause, values } = buildWhere([
...
router.get("/monitoring/upload-status", validate(monitoringUploadQuery, "query"), async (req, res, next) => {
  try {
    ...
    const orderBy = buildOrderBy(req.query.sort_by, req.query.sort_order, ["teacher_name", "student_name", "scan_uploaded_at", "report_uploaded_at"], "teacher_name");
    ...
    const [dataResult, countResult] = await Promise.all([
      ...
      query(`SELECT COUNT(*)::int AS total FROM vw_teacher_upload_status ${whereClause}`, values),
    ]);
...
router.get("/monitoring/activity", validate(monitoringActivityQuery, "query"), async (req, res, next) => {
  try {
    ...
    const result = await query(`SELECT * FROM vw_monthly_center_activity ${whereClause} ORDER BY month DESC, center_name`, values);
...
//
// [FIX] Versi sebelumnya tidak ada LIMIT/OFFSET — query return semua row
// sekaligus tanpa batas. Jika enrollment mencapai ribuan, ini menyebabkan:
//   1. Memory spike di Node.js (semua row di-load ke memori sekaligus)
//   2. Response timeout karena query lama
//   3. Payload response sangat besar, lambat di client
//
// Fix: tambah pagination. Default limit 100 (lebih besar dari API lain
// karena ini endpoint download/export), max tetap 100 sesuai MAX_LIMIT.
// Jika butuh export semua data, lakukan iterasi di sisi client atau
// gunakan streaming endpoint tersendiri.
router.get("/download/enrollments", validate(downloadEnrollmentsQuery, "query"), async (req, res, next) => {
  try {
    ...
    const [dataResult, countResult] = await Promise.all([query(dataQuery, [...values, limit, offset]), query(countQuery, values)]);
